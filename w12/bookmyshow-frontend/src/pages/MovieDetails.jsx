...
export default function MovieDetails() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        ...
        const showsResponse = await getShows({ movieId: id });
  ...
  function handleBook(show) {
    navigate("/bookings", { state: { movie, show } });
  }
    ...
    <section>
      ...
      {shows.length === 0 ? (
        ...
        <div style={styles.grid}>
          {shows.map((show) => <ShowCard key={show._id} show={show} onBook={handleBook} />)}
        </div>
      )}
    </section>
  );
...
const styles = { grid: { display: "grid", gap: "20px", marginTop: "20px" } };

...
function createBooking(req, res, next) {
    try {
        ...
        const showtime = movies.showtimes.find((s) => s.id === Number(showtimeId.movieId));
        if (!showtime) {
...
function getAllBookings(req,res,next){
    try {
        res.status(200).json({
            success: true, count:bookings.length, dat:bookings
        })
    } catch (error) {
        next(error);
    }
...
function getMyBookings(req,res,next){
    try {
        ...
        res.status(200).json({
            success: true, count:userBookings.length, dat:userBookings
        })
    } catch (error) {
        next();
    }
